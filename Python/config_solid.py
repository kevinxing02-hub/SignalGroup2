...
# --------------------------
# If True, use Leave-One-Group-Out aggregation when selecting stable features.
LOSO_ENABLED = True

# Optional: maximum number of LOSO folds to actually run.
# If you have many subjects and want to limit compute, set to e.g. 30.
# Set to None to run all folds.
LOSO_MAX_FOLDS = None

# Stability threshold: fraction of folds a feature must appear in to be kept.
# 1.0 = keep only features selected in every fold (strict). 0.8 = keep features
# selected in 80% of folds (more permissive).
FEAT_STABILITY_THRESHOLD = 1.0

# If you prefer to provide groups through the config rather than passing them
# to the function, set GROUPS to an array-like of length n_samples. Otherwise
# leave as None and pass groups to select_features(features, labels, config, groups).
GROUPS = None

# Nonlinear features (Iteration 4 final)
ENABLE_NONLINEAR_FEATURES = True      # master switch
NONLINEAR_PER_CHANNEL = False        # True = compute per EEG channel; False = compute once per epoch (aggregate)
NONLINEAR_MIN_EPOCH_LEN = 30        # minimum samples required to compute (fallback to nan otherwise)
# optional: list of features to compute; can be used later to control compute
NONLINEAR_FEATURES = [
